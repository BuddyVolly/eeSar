...
from ..misc import _terrain_flattening
...
def create(
    ...
    def add_ratio_band(image):
        return image \
             .addBands(image.select('VV').subtract(image.select('VH')).rename('VVVH_ratio'))

    def add_ND_ratio_band(image):
        return image \
             .addBands(image.normalizedDifference(['VV', 'VH']).rename('VVVH_ND_ratio'))

    def pre_process(image):
        steps = []
        #bandList = image.get('transmitterReceiverPolarisation')
